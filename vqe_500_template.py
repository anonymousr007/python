...
def find_excited_states(H):
    ...
    def variational_ansatz2(params, wires):
        ...
        if n_rotations > 1:
            ...
            # For 1-qubit case, just a single rotation to the qubit
            qml.Rot(*params[0], wires=wires[0])
    def variational_ansatz3(params, wires):
        angles = params[0]
        params = params[1:]
        n_qubits = len(wires)
        n_rotations = len(params)
    ...
    cost_fn3 = qml.ExpvalCost(variational_ansatz3, H, dev)
    max_iterations = 1000
    conv_tol = 1e-08
    
    def cost_fn(params):
         return 2.0*cost_fn0(params) + 1.5*cost_fn1(params) + 0.75*cost_fn2(params) + 0.25*cost_fn3(params)
    ...
    for n in range(max_iterations):
        ...
        if conv <= conv_tol:
            break
    spectrum[0] = cost_fn0(params)
    spectrum[1] = cost_fn1(params)
    spectrum[2] = cost_fn2(params)
    spectrum[3] = cost_fn3(params)
